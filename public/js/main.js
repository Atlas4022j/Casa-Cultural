...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Optimización del evento scroll con requestAnimationFrame
    let ticking = false;
    ...
    whatsappButton.addEventListener('mousedown', () => {
        whatsappButton.style.transform = 'scale(0.95) translateY(2px)';
    });
    ...
    whatsappButton.addEventListener('touchend', () => {
        whatsappButton.style.transform = 'scale(1) translateY(0)';
    });

    // Verificar si el dispositivo tiene WhatsApp instalado
    whatsappButton.addEventListener('click', function(e) {
        if (/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
            const testLink = document.createElement('a');
            testLink.href = 'whatsapp://';
            testLink.addEventListener('error', function(error) {
                e.preventDefault();
                alert('Por favor, instala WhatsApp para continuar.');
            });
        }
    });
